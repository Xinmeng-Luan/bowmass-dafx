...
class hessian():
    ...
    def __init__(self, model,  data=None, cuda=True):
        """
        ...
        p,  q = self.model(self.inputs[0], self.inputs[1])
        self.f = 100
        ...
        p_t = (torch.autograd.grad(p, data, grad_outputs=torch.ones_like(p), create_graph=True)[0])
        q_t = (torch.autograd.grad(q, data, grad_outputs=torch.ones_like(q), create_graph=True)[0])
